#!/bin/bash
# Resume v24b from epoch 60 after CUDA crash, then continue to v24c. Authorized.
set -e
cd /home/mmann1123/extra_space/bcm_emulator
mkdir -p logs

echo "[chain] resume v24b from epoch_060 at $(date)"
PYTHONUNBUFFERED=1 conda run --no-capture-output -n deep_field python train.py \
    ...
    --resume checkpoints/checkpoint_epoch060.pt \
    --notes "v17-polaris-awc + annual-pooled MSE on PET+AET, lambda=3.0; resumed from epoch 60 after CUDA unspecified launch failure" \
    >> logs/v24b.log 2>&1
